...
def test_post_of_good_data(client, age, bio):
    ...
    assert (
        "<h1>Python Flask Practice Assessment</h1>" in content
    ), f"Failed on age={age}, bio={bio}"
...
def test_simple_form_data_retrieval(client):
    ...
    with app.app_context():
        ...
        db.session.add(
            SimplePerson(name="Marley", age=21, bio="Living life")
        )
        db.session.add(
            SimplePerson(
                name="Harley", age=100, bio="Rocking chair demon"
            )
        )
        db.session.commit()
